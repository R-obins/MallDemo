        ...
        <div class="rs_content">
        	...
        	<input type="hidden" id="id" name="id" >
	            ...
	            <div  class="address_content">
					 <span class="red">*</span><span class="kuan">省&nbsp;&nbsp;份：</span><select data-province="---- 选择省 ----" id="receiverState" name="receiverState" onchange="getCityFun(this.value,-1,-1)"></select>
...
<script type="text/javascript">
				...
				for(i=0;i<obj.data.length;i++){
					if(obj.data[i].isDefault==1){
	                    ...
	                    '<span class="operation operation_normal">'+
	                    	'<span class="aco_change" onclick="goUpdateFun('+obj.data[i].id+')">修改</span>|<span class="aco_delete" onclick="deleteFun('+obj.data[i].id+')">删除</span>'+
	                    '</span>'+
	...
	function getProvinceFun(provinceCode,cityCode,areaCode){
		$.ajax({
			...
			"dataType":"json",
			"success":function(obj){
				$("#receiverState").html("<option>. . . 请求选择 . . .</option>");
				for(i=0;i<obj.data.length;i++){
					var op = 
						new Option(obj.data[i].provinceName,obj.data[i].provinceCode);
	...
	$(function(){
	    ...
	    // 把账号管理的图片设置为和其它上个不同myOrder1.png
