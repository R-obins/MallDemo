...
<div id="container" class="clearfix">
    ...
    <div class="rightsidebar_box rt">
        <div class="rs_header">
            <span >
            <a href="../user/showPersonInfo.do">
            	我的信息
            </a></span>
            <span class="rs_header_active">
            <a href="../user/showPassword.do">
            	安全管理
            </a></span>
        </div>
...
function btnUpdatePassword(){
	...
	if(checkPasswordLength(oldVal)&&
		checkPasswordLength(newVal)&&
		checkPasswordEquals()){
		$.ajax({
			"url":"../user/updatePassword.do",
			"data":"oldPassword="+oldVal+"&newPassword="+newVal,
			"type":"POST",
			"dataType":"json",
...
$("#confirmPassword").blur(function(){
	...
	//函数返回true.表示输入的密码在6~9位之间,
	//说明格式是正确的
	if(checkPasswordLength(confirmVal)){
		$("#confirmPasswordSpan").html("密码格式正确");
		$("#confirmPasswordSpan").css("color","green");
		//判断两次输入的密码是否相同
		if(checkPasswordEquals()){
			$("#confirmPasswordSpan").html("确认密码和新密码相同");
			$("#confirmPasswordSpan").css("color","green");
		}else{
			$("#confirmPasswordSpan").html("两次输入密码不相同");
			$("#confirmPasswordSpan").css("color","red");
		}
	}else{
		$("#confirmPasswordSpan").html("密码格式不正确");
		$("#confirmPasswordSpan").css("color","red");
	}
});
